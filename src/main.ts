...
async function SENDGRID_CATEGORY_STATS(
  ...args: GoogleSheetsSendgridFunctionResolver.CategoryStatsArgs
) {
...
async function SENDGRID_GLOBAL_STATS(
  ...args: GoogleSheetsSendgridFunctionResolver.GlobalStatsArgs
) {
  const sendgridApiKey = scriptProperties.getProperty('SENDGRID_API_KEY');
  if (sendgridApiKey == null) throw new Error('SENDGRID_API_KEY is not set');
